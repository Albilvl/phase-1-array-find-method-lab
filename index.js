...
//   const winner = record.find((records)=>{ //arrow function that goes through each item (record)
//     records.result === `W` // returns a match if result is `W`
//   })

 
  function superbowlWin (teamyear){ // teamyear is anyparameter i want
    const winningRecord = teamyear.find(yearData => yearData.result === `W`)
    return winningRecord ? winningRecord.year : undefined
}

// ^ basically defined winningrecord  is defined and the function .find() is a callback
//we used ternary in return if winning record, then execute winning record result else retrun undefined